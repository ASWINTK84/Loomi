...
export const CartProvider = ({ children }) => {
 ...
 const [loadingCart, setLoadingCart] = useState(false); // Keep for initial load and full clear
...
    setLoadingCart(true); // Only set loading for initial fetch
    setCartError(null);
    try {
      const config = {
        headers: {
...
    } finally {
      setLoadingCart(false); // Only unset loading after initial fetch
    }
  }, [isLoggedIn, token]);
...
    // setLoadingCart(true); // Don't show loading overlay for add to cart
    setCartError(null);
    try {
      const config = {
        headers: {
...
    } // No finally block to set loadingCart to false here
  };
...
    const updatedTotal = updatedItems.reduce((acc, item) => acc + item.subTotal, 0);

    setCartItems(updatedItems);
    setCartTotal(updatedTotal);
    toast.info('Removing item...'); // Provide immediate feedback
...
      if (data.success) {
        // Backend's response is the source of truth, but we already updated optimistically
        // So, no need to set `cartItems` and `cartTotal` again unless there's a discrepancy.
        // For simplicity, we can still set them to ensure exact sync if backend recalculates slightly differently.
        setCartItems(data.items);
        setCartTotal(data.total);
...
      return;
    }
    if (newQuantity < 1) { // Changed from < 0 to < 1 since we're handling removal separately or via confirm
        toast.error('Quantity cannot be less than 1. Use the trash icon to remove.');
        return;
    }

    // Optimistic update: Update the quantity in the UI immediately
    const originalCartItems = cartItems;
    const originalCartTotal = cartTotal;

    const updatedItems = cartItems.map(item =>
      item._id === cartItemId
        ? { ...item, quantity: newQuantity, subTotal: (item.product.offerPrice || item.product.price) * newQuantity }
        : item
    );
    const updatedTotal = updatedItems.reduce((acc, item) => acc + item.subTotal, 0);
...
      if (data.success) {
        // Backend's response is the source of truth; re-sync to be safe
        setCartItems(data.items);
        setCartTotal(data.total);
...
      toast.error(error.response?.data?.message || 'Error updating cart quantity.');
    }
  };
...
    setLoadingCart(true); // Still show loading for a full cart clear
    setCartError(null);
    try {
      const config = {
        headers: {
